...
def create_access_token(user: User) -> str:
    iat = _now_ts()
    lifetime = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)  # FIXED: removed erroneous "* 60"
    payload = {
        "sub": str(user.id),
        "org": user.org_id,
        "role": user.role,
...
def get_token_payload(request: Request) -> dict:
    ...
    if payload.get("jti") in _revoked_tokens:  # FIXED: was checking "sub" instead of "jti"
